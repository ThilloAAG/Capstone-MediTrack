...
export default function DashboardScreen() {
  ...
  useEffect(() => {
    const user = auth.currentUser;
    if (!user) {
      setAdherence({ compliancePct: 0, missedDoses7d: 0 });
      return;
    }
    ...
    const unsubscribe = onSnapshot(
      ...
      (snapshot) => {
        ...
        snapshot.docs.forEach((docSnap) => {
          ...
          if (!actualTime || actualTime < since) return;
        ...
        const compliancePct = total > 0 ? Math.round((taken / total) * 100) : 0;
      ...
      (error) => {
        console.log("Load adherence logs error:", error);
        setAdherence({ compliancePct: 0, missedDoses7d: 0 });
      }
    );
        ...
        >
          <View style={styles.upcomingDosesContainer}>
            ...
            {nextTwoHoursDoses.length > 0 ? (
              ...
              >
                {nextTwoHoursDoses.map((dose, index) => (
                  <View
                    key={dose.id}
                    style={[
                      styles.doseCard,
                      index === nextTwoHoursDoses.length - 1 && styles.lastDoseCard,
                    ]}
                  >
          ...
          <View style={styles.healthInsightsContainer}>
            ...
            <View style={styles.insightsGrid}>
              ...
              <View style={styles.insightCard}>
                ...
                <Text style={styles.insightValue}>{adherence.compliancePct}%</Text>
                <Text style={styles.insightLabel}>Compliance</Text>
              </View>

              <View style={styles.insightCard}>
                <Ionicons name="close-circle" size={20} color="#ef4444" />
                <Text style={styles.insightValue}>{adherence.missedDoses7d}</Text>
                <Text style={styles.insightLabel}>Missed 7d</Text>
              </View>

              <View style={styles.insightCard}>
                <Ionicons name="time" size={20} color="#f59e0b" />
                <Text style={styles.insightValue}>{totalUpcomingDoses}</Text>
                <Text style={styles.insightLabel}>Upcoming 24h</Text>
              </View>
            </View>
          </View>
...
const styles = StyleSheet.create({
  ...
  insightValue: { fontSize: 22, fontWeight: "700", color: "#111618", marginTop: 8 },
  ...
  addDoctorButtonText: { color: "#ffffff", fontSize: 14, fontWeight: "800", marginLeft: 8 },
